...
,
    ...
    /* ---- GLM 5.2 additions: 14 structurally distinct specimens across 13 drawers (chip #eab308) ---- */
...

    {
      ...
      section: "effects",
      name: "Brushed Metal",
      description: "Anisotropic milled-aluminum plate with engraved lettering and a traveling specular sweep — a mechanical finish unlike every glow, grain, glass, foil, and grid effect.",
      creator: "glm-5-2",
      tags: ["effect","brush","metal","aluminum","specular","industrial"],
      tweaks: [
        { type: "color", label: "Metal Tone", varName: "--glm-ef11", default: "#9aa3b2" },
        { type: "range", label: "Sweep Speed", varName: "--glm-ef11-speed", min: 1, max: 8, step: 1, unit: "s", default: 4 }
      ],
      code: "<style>\n  @keyframes kglm-sweep{0%,55%{transform:translateX(-160%)}85%,100%{transform:translateX(460%)}}\n  .kglm-metal{position:relative;width:190px;height:72px;border-radius:8px;overflow:hidden;border:1px solid rgba(255,255,255,.14);box-shadow:inset 0 1px 0 rgba(255,255,255,.35),0 10px 20px rgba(0,0,0,.45);background:linear-gradient(180deg,rgba(255,255,255,.16),rgba(255,255,255,0) 22%,rgba(255,255,255,.04) 55%,rgba(0,0,0,.3)),repeating-linear-gradient(90deg,rgba(255,255,255,.06) 0 1px,rgba(255,255,255,0) 1px 3px),linear-gradient(180deg,var(--glm-ef11,#9aa3b2) 0%,#3a404c 55%,#23262e 100%)}\n  .kglm-metal::before{content:\"\";position:absolute;inset:0;background:radial-gradient(circle at 30% 20%,rgba(255,255,255,.18),transparent 55%)}\n  .kglm-metal::after{content:\"\";position:absolute;top:-24%;bottom:-24%;left:0;width:34%;background:linear-gradient(90deg,transparent,rgba(255,255,255,.22),transparent);animation:kglm-sweep var(--glm-ef11-speed,4s) ease-in-out infinite}\n  .kglm-metal b{position:relative;z-index:1;font:800 13px/1 ui-monospace,Consolas,monospace;letter-spacing:.34em;color:#f3f6fa;text-shadow:0 -1px 2px #0b0d12,0 1px 0 rgba(255,255,255,.45)}\n  @media (prefers-reduced-motion:reduce){.kglm-metal::after{animation:none}}\n</style>\n<div class=\"kglm-metal\"><b>GLM 5.2</b></div>"
    }
