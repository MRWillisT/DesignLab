...
window.AGENT_PROMPT = [
  ...
  "4. Sign every new item with the name and chip color listed above. Before adding anything, register yourself in the creators array at the top of js/data.js ({ id, name, color }) using a short lowercase id derived from your name. Pick a chip color no existing creator already uses - the prompt studio filters taken colors and the gate warns on duplicates. The reserved id \"me\" belongs to the local user - never sign with it.",
  ...
  "Your push is blocked if the pre-push gate fails, so verify before committing:",
  "  node --check js/data.js && node --check js/app.js",
  "  node scripts/check-registry.mjs   (structural rules + behavioral smoke test; must pass)",
  "",
  "SUBMISSION (for outside contributors — you do NOT have push access to the main repo):",
  "  1. Fork https://github.com/MRWillisT/DesignLab on GitHub (one-time).",
  "  2. Clone your fork: git clone https://github.com/<YOUR-USER>/DesignLab.git",
  "  3. Create a branch: git checkout -b <drawer-code>-add-<your-name>",
  "  4. Make your changes (batch file + data.js creator entry + index.html script tag).",
  "  5. Verify: node --check js/data.js && node --check js/app.js && node scripts/check-registry.mjs",
  "  6. Commit: git add js/data.js js/items/<batch>.js index.html && git commit -m \"<drawer-code>: add <IDs> (<short names>)\"",
  "  7. Push: git push origin <branch-name>",
  "  8. Open a pull request from your fork's branch to MRWillisT/DesignLab:main on GitHub.",
  "  The CI check runs the same registry gate and will block the merge if anything is broken.",
  "",
  "If you cannot fork or prefer a lighter path, paste your finished items array into the submission textarea on the site — it validates your JSON and opens a pre-filled GitHub issue.",
  "",
  "WHERE SPECIMENS LIVE: small additions belong in js/data.js; larger batches and whole new drawers belong in a js/items/<batch-name>.js file that pushes onto window.DESIGN_LAB.items (AGENTS.md Option C) and is wired into index.html before app.js. A new drawer costs one sections line in js/data.js plus a batch file. The gate validates batch files exactly like data.js and names the batch in its errors.",
  "Never use --no-verify to bypass the gate, never force-push, never amend or delete other agents' commits.",
  "",
