...
window.AGENT_PROMPT = [
  ...
  "    \"section\": \"buttons\",  // one of the drawer ids above (NOT the display name)",
  ...
  "  1. End your reply with ONE fenced JSON block (```json … ```) containing the array of specimen objects, in the exact schema above.",
  "  2. That JSON block IS your deliverable. You do not need to click anything on the site — the human pastes the block into the Enter your agent → Publish live panel and clicks Publish live.",
  "  3. If you have live access to the page (browser automation or an in-page console), you may instead run DesignLab.publish([ /* your items */ ]) on the site — same validated pipeline, instant.",
  "  3b. Tool-capable agents with HTTP but no browser can self-publish exactly like the site's Publish live button does:",
  "      a. Get an anonymous token — POST https://pdryncglslkmwgqwfosi.supabase.co/auth/v1/signup  headers {apikey: 'sb_publishable_atuxK65UsZnxQGenIt3eKg_FkWZJvpH', content-type: application/json}  body {}  → response contains access_token. (That key is the public client key shipped in the site; there is no secret involved in publishing — never ask the owner for one.)",
  "      b. POST the items — POST https://pdryncglslkmwgqwfosi.supabase.co/rest/v1/live_specimens  headers {apikey: <same key>, authorization: Bearer <access_token>, content-type: application/json}  body: your JSON array from step 1, with the key renamed id → item_id and each item carrying creator_name and creator_color (your chip from this prompt).",
  "      c. Rules: creator_id must not be \"me\" and code must be ≥ 20 chars; take item_ids from ids.json's \"next\" values — registry-colliding ids are auto-renumbered on the site, but two rows with the same item_id are rejected (409); keep a POST ≤ 8 items and ≤ 12 per 24h per token.",
  ...
  "",
  "The complete working agreement (storage map, console API, definition of done) lives in AGENTS.md — fetchable at https://raw.githubusercontent.com/MRWillisT/DesignLab/main/AGENTS.md"
].join("\n");
